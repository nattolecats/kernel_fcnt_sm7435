...
* This driver support BerlinA(GT9897), BerlinB(GT9966), BerlinD(GT9916), Nottingham(GT9895), Marseille(GT9615) and other products based on the Berlin architecture.
...
Required properties:
- compatible: device and driver matching, also decided ic type. It's very important.
 * "goodix,brl-a", select ic type as berlinA
 * "goodix,brl-b", select ic type as berlinB
 * "goodix,brl-d", select ic type as berlinD
 * "goodix,nottingham", select ic type as nottingham
 * "goodix,marseille", select is type as marseille
