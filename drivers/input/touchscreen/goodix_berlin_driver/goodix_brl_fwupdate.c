		...
		struct firmware_data *fw_data)
{
	...
	get_hardware_info_data(HWID_CTP_DRIVER, fw_summary->fw_pid);
	...
	ts_info("Firmware vid:0x%02X", fw_summary->fw_vid[0]);
    ts_info("Firmware vid:0x%02X", fw_summary->fw_vid[1]);
    ts_info("Firmware vid:0x%02X", fw_summary->fw_vid[2]);
    ts_info("Firmware vid:0x%02X", fw_summary->fw_vid[3]);

    sprintf(hex_str[0], "%02X", fw_summary->fw_vid[0]);
    sprintf(hex_str[1], "%02X", fw_summary->fw_vid[1]);
    sprintf(hex_str[2], "%02X", fw_summary->fw_vid[2]);
    sprintf(hex_str[3], "%02X", fw_summary->fw_vid[3]);

    strcat(hex_str[0], hex_str[1]);
    strcat(hex_str[0], hex_str[2]);
    strcat(hex_str[0], hex_str[3]);
     
    ts_info("Hex number is represented as string %s.\n", hex_str);
  
    get_hardware_info_data(HWID_CTP_FW_INFO, hex_str);
