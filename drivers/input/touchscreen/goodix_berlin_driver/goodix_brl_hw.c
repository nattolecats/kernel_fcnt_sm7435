...
static int brl_power_on(struct goodix_ts_core *cd, bool on)
{
	...
	if (on) {
		...
		//msleep(GOODIX_SHORT_RESET_DELAY_MS);
...
int brl_glove(struct goodix_ts_core *cd, int glove_type)
{
	struct goodix_ts_cmd cmd;
	if (cd->bus->ic_type == IC_TYPE_BERLIN_A)
		cmd.cmd = GOODIX_GESTURE_CMD_BA;
	...
	cmd.data[0] = glove_type & 0xFF;
	cmd.data[1] = (glove_type >> 8) & 0xFF;
	if (cd->hw_ops->send_cmd(cd, &cmd))
...
int brl_charger(struct goodix_ts_core *cd, int charger_type)
{
	struct goodix_ts_cmd cmd;
	...
	cmd.data[0] = charger_type & 0xFF;
	cmd.data[1] = (charger_type >> 8) & 0xFF;
	if (cd->hw_ops->send_cmd(cd, &cmd))
...
int brl_edge(struct goodix_ts_core *cd, int edge_type)
{
	struct goodix_ts_cmd cmd;
	...
	cmd.data[0] = edge_type & 0xFF;
	cmd.data[1] = (edge_type >> 8) & 0xFF;
	if (cd->hw_ops->send_cmd(cd, &cmd))
...
static int brl_esd_check(struct goodix_ts_core *cd)
{
	...
	ts_err("esd_check start");
	if(spi_flag == 0)
	{
		cd->hw_ops->reset(cd, 5);
		goodix_ts_power_off(cd);
		usleep_range(5000, 5100);
		goodix_ts_power_on(cd);
		ts_err("spi read error,need reset");
		spi_flag = 1;
	}
