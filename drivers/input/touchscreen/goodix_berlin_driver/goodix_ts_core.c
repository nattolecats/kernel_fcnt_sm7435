...
static bool first_tp_pull = false;
...
static void goodix_device_manager_exit(void)
{
	...
	if (!list_empty(&goodix_devices.list)) {
		list_for_each_entry_safe(res, next, &goodix_devices.list, list) {
			ts_info("platform device[%s] unregister", res->name);
...
static void goodix_ts_charger_work(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct goodix_ts_charger *ts_charger = container_of(dwork,
			struct goodix_ts_charger, charger_work);
	struct goodix_ts_core *cd = container_of(ts_charger,
			struct goodix_ts_core, ts_charger);
	const struct goodix_ts_hw_ops *hw_ops = cd->hw_ops;
	...
	if (cd->chargerswitch){
		if (is_tp_on()) {
			if (!cd->chargertype){
			ts_info("enable charger tap");
			cmd.cmd = GOODIX_CHARGER_CMD;
			cmd.len = 5;
			cmd.data[0] = 0x01;
			cmd.data[1] = 0xFF;
			ts_info("charge enabled1");
			cd->chargertype = true;
		if (hw_ops->send_cmd(cd, &cmd))
			ts_err("failed send charger cmd");
			} 
		}

		if (!is_tp_on()) {
			if (cd->chargertype){
			ts_info("disable charger tap");
			cmd.cmd = GOODIX_CHARGER_CMD;
			cmd.len = 5;
			cmd.data[0] = 0x00;
			cmd.data[1] = 0xFF;
			ts_info("charge enabled2");
			cd->chargertype = false;
		if (hw_ops->send_cmd(cd, &cmd))
			ts_err("failed send charger cmd");
			}
		}
	...
	if (!atomic_read(&ts_charger->charger_on))
		return;

	if (atomic_read(&ts_charger->charger_on))
		schedule_delayed_work(&ts_charger->charger_work, 1 * HZ);
}
...
/**
 * goodix_ts_charger_on - turn on charger protection
 */
void goodix_ts_charger_on(struct goodix_ts_core *cd)
{
	struct goodix_ts_charger *ts_charger = &cd->ts_charger;

	if (atomic_read(&ts_charger->charger_on))
		return;

	atomic_set(&ts_charger->charger_on, 1);
	if (!schedule_delayed_work(&ts_charger->charger_work, 1 * HZ))
		ts_info("charger work already in workqueue");
...
/**
 * goodix_ts_charger_off - turn off charger protection
 */
void goodix_ts_charger_off(struct goodix_ts_core *cd)
{
	struct goodix_ts_charger *ts_charger = &cd->ts_charger;
	int ret;
	...
	atomic_set(&ts_charger->charger_on, 0);
	ret = cancel_delayed_work_sync(&ts_charger->charger_work);
	ts_info("charger off");
}
...
/**
 * goodix_ts_charger_init - initialize charger protection
 */
int goodix_ts_charger_init(struct goodix_ts_core *cd)
{
	struct goodix_ts_charger *ts_charger = &cd->ts_charger;

	INIT_DELAYED_WORK(&ts_charger->charger_work, goodix_ts_charger_work);
	ts_charger->ts_core = cd;
	atomic_set(&ts_charger->charger_on, 0);
	goodix_ts_charger_on(cd);
...
static bool allow_tp_resume_in_aod = false;
...
int goodix_ts_resume(struct goodix_ts_core *core_data)
{
	...
	ts_info("aodtype:%d, allow_tp_resume_in_aod:%d", aodtype, allow_tp_resume_in_aod);	
	...
	if (glove_switch == true){
		ts_info("enable glove tap true");
		...
		core_data->glove_type |= GESTURE_GLOVE_TAP;

		if (core_data->hw_ops->send_cmd(core_data, &cmd)) 
			ts_err("failed send glove cmd");
		}

	core_data->chargertype = false;
	core_data->chargerswitch = true;
	return 0;
}
		 ...
		 struct panel_event_notification *notification, void *client_data)
{
	...
	ts_debug("double_tap:%d, aodtype:%d, first_tp_pull:%d\n", core_data->double_tap_mode, aodtype, first_tp_pull);
	if (core_data->double_tap_mode) {
		//double tap mode resume tp
	   if(aodtype && first_tp_pull) {
			ts_debug("double tap resume tp A \n");
			goodix_ts_resume(core_data);
			first_tp_pull = false;
	   }
	   aodtype = false;
	   core_data->double_tap_mode = false;
	   ts_debug("enter double tap A\n");
	} else {
	   core_data->double_tap_mode = false;
	   if(!aodtype && first_tp_pull) {
			ts_debug("lcd power on resume tp when exit aod A\n");
			goodix_ts_resume(core_data);
			first_tp_pull = false;
	   }
	}
...
{
  	struct goodix_ts_core *cd = container_of(self,struct goodix_ts_core, ts_notifier);
	const struct goodix_ts_hw_ops *hw_ops = cd->hw_ops;
...
static int goodix_ts_probe(struct platform_device *pdev)
{
	...
	if (IS_ENABLED(CONFIG_OF) && bus_interface->dev->of_node) {
		...
		of_property_read_string(bus_interface->dev->of_node, "qcom,touch-environment", &core_data->touch_environment);
...
static int goodix_ts_remove(struct platform_device *pdev)
{
	...
	ts_info("platform device[%d] remove", pdev->id);
