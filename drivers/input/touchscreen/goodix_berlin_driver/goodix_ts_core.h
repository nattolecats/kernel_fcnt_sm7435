...
#define GOODIX_DRIVER_VERSION			"v1.3.16"
...
#define GOODIX_SHORT_RESET_DELAY_MS	10
#define GOODIX_VSHORT_RESET_DELAY_MS	1
...
enum GOODIX_GESTURE_TYP {
	...
	GESTURE_GLOVE_TAP   = (1 << 14),
	GESTURE_CHARGER_TAP   = (1 << 14),
	GESTURE_EDGE_TAP   = (1 << 14),
};
...
struct goodix_ts_hw_ops {
	...
	int (*glove)(struct goodix_ts_core *cd, int glove_type);
	int (*charger)(struct goodix_ts_core *cd, int charger_type);
	int (*edge)(struct goodix_ts_core *cd, int edge_type);
...
extern int goodix_ts_power_on(struct goodix_ts_core *cd);
