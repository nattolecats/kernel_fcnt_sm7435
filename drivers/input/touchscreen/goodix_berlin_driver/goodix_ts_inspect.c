...
static int goodix_ito_result(struct goodix_ts_test *ts_test)
{
	int i;
	bool result = false;
...
static void goodix_ito_result_data(struct goodix_ts_test *ts_test)
{
	int ret;

	ret = goodix_ito_result(ts_test);
	if (ret < 0)
		return;
}
...
static int goodix_ito_inspect(struct goodix_ts_core *cd)
{
	int ret;
	...
	goodix_ito_result_data(ts_test);
...
static ssize_t ito_test_write(struct file *file, const char __user *buf,
...
static int ito_test_open(struct inode *inode, struct file *file)
{
	struct seq_file *seq_file;
	int ret;
...
static const struct proc_ops ito_test_ops = {
	.proc_open = ito_test_open,
	.proc_read = seq_read,
	.proc_write = ito_test_write,
	.proc_lseek = seq_lseek,
	.proc_release = seq_release,
};
...
int inspect_module_init(struct goodix_ts_core *core_data)
{
	...
	proc_entry = proc_create_data("ito_test",
			0660, core_data->proc_dir_entry, &ito_test_ops, core_data);
	if (!proc_entry) {
        ts_err("failed to create proc entry");
        return -ENODEV;
    }
...
void inspect_module_exit(struct goodix_ts_core *core_data)
{
	...
	ts_info("inspect module exit");
	remove_proc_entry("ito_test", core_data->proc_dir_entry);
}
