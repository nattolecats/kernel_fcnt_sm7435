...
void get_hardware_info_data(enum hardware_id id, const void *data)
{
	if (NULL == data) {
		...
		case HWID_BATERY_ID_OHM:
			...
			break;
		case HWID_PPG_SENSOR:
			hwinfo_data.ppg = data;
			break;
		default:
			hwinfo_debug("[HWINFO] %s Invalid HWID\n", __func__);
			break;
...
static ssize_t show_ppg_id(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	if (NULL != hwinfo_data.ppg) {
		return sprintf(buf, "ppg name :%s\n",
			       hwinfo_data.ppg);
	} else {
		return sprintf(buf, "ppg name :Not Found\n");
	}
}
...
static unsigned int get_ram_size(void)
{
	...
	if (temp_size > 23552) {
		ram_size = 24;
	} else if (temp_size > 15360) {
		ram_size = 16;
	} else if (temp_size > 11264) {
		ram_size = 12;
...
static DEVICE_ATTR(ppg, 0444, show_ppg_id, NULL);
