...
static char boot_mode[20];
module_param_string(mode, boot_mode, sizeof(boot_mode),0644);

char *get_charge_status_for_nfc(void){
	pr_err("NFC: get_charge_status (%s)\n", boot_mode);
	return boot_mode;
}
...
int nfc_i2c_dev_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	char *charge_status_for_nfc = NULL;

	pr_debug("%s: enter\n", __func__);
	charge_status_for_nfc = get_charge_status_for_nfc();

	if(strstr(charge_status_for_nfc,"charger") != NULL) {
		struct device *dev = &client->dev;
		nfc_gpio->ven = -EINVAL;
		nfc_gpio->ven = of_get_named_gpio(dev->of_node, DTS_VEN_GPIO_STR, 0);
