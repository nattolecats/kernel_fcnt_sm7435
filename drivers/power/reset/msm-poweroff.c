...
*/
	set_dload_mode(download_mode &&
			(((cmd != NULL && cmd[0] != '\0') && (
				!strncmp(cmd, "warm", 4) ||
				!strncmp(cmd, "dm-verity device corrupted", 26) ||
				!strncmp(cmd, "SIM is added.", 13)
...
*/
		// Nagoya LIMITED For SELinux by sunning at 2023-12-26 start
		} else if (!strncmp(cmd, "selinux-permissive", 18)) {
			reason = PON_RESTART_REASON_SELINUX_PERMISSIVE;
			__raw_writel(0x6f656d10, restart_reason);
		} else if (!strncmp(cmd, "selinux-enforcing", 17)) {
			reason = PON_RESTART_REASON_SELINUX_ENFORCING;
			__raw_writel(0x6f656d11, restart_reason);
		// Nagoya LIMITED For SELinux by sunning at 2023-12-26 end
		} else if (!strncmp(cmd, "offcharge", 9)) {
			reason = PON_RESTART_REASON_OFFCHARGE;
			__raw_writel(0x6f656d01, restart_reason);
		} else if (!strncmp(cmd, "forcenormal", 11)) {
			reason = PON_RESTART_REASON_FORCENORMAL;
			__raw_writel(0x6f656d02, restart_reason);
		} else if (!strncmp(cmd, "User's request", 14) ||
					!strncmp(cmd, "userrequested", 13)) {
			reason = PON_RESTART_REASON_USERS_REQUEST;
			__raw_writel(0x6f656d03, restart_reason);
		} else if (!strncmp(cmd, "SIM is added.", 13)) {
			reason = PON_RESTART_REASON_SIM_IS_ADDED;
			__raw_writel(0x6f656d06, restart_reason);
		} else if (!strncmp(cmd, "shipmode", 8)) {
			reason = PON_RESTART_REASON_SHIPMODE;
			__raw_writel(0x6f656d07, restart_reason);
		} else {
			reason = PON_RESTART_REASON_OTHER_REQUEST1;
			__raw_writel(0x77665501, restart_reason);
		}
	} else {
		if (in_panic) {
			reason = PON_RESTART_REASON_OTHER_REQUEST2;
			__raw_writel(0x77665501, restart_reason);
		} else {
			reason = PON_RESTART_REASON_USER_SPACE_REQUEST;
