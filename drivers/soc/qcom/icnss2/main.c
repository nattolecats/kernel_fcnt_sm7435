...
#include "power.h"
#include "genl.h"
#define WLAN_DUMP_CREATED 1
static int wlan_dump_flags = 0;
	...
	while (!list_empty(&priv->event_list)) {
		...
		case ICNSS_DRIVER_EVENT_FW_READY_IND:
			ret = icnss_driver_event_fw_ready_ind(priv,
								 event->data);
			if (wlan_dump_flags == WLAN_DUMP_CREATED) {
				if (kobject_uevent(&priv->pdev->dev.kobj, KOBJ_CHANGE)) {
					icnss_pr_err("set the wlan dump event fail");
				}
				wlan_dump_flags = 0;
				icnss_pr_dbg("wlan is crash, collect dump files");
			}
			break;
		case ICNSS_DRIVER_EVENT_REGISTER_DRIVER:
...
static void icnss_pdr_notifier_cb(int state, char *service_path, void *priv_cb)
{
	...
	case SERVREG_SERVICE_STATE_DOWN:
		...
		wlan_dump_flags = WLAN_DUMP_CREATED;
